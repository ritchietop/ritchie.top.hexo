...
2. 利用排序模型对上一步得到的视频库子集进行打分排序，选择TopN个视频推荐给用户。（在线细筛）
...
> 用户观察视频列表，用户历史搜索记录列表，用户性别，用户年龄，用户地域，行为访问设备，用户登录状态，视频上传时间，视频ID
...
**视频上传时间**：连续特征可以直接输入到模型中，也可以通过Embedding的方式处理

**视频ID**：进行Embedding处理

> ### 关于视频上传时间的说明
> 
> 在视频领域，通常来说，用户更倾向于点击新上传的视频。基于这个特点，论文中将视频上传时间加入到了模型中，以期达到消除视频上传时间引入的偏执
> 
> {% asset_img video_upload_time.png 视频上传时间 %}
> 
> 加入**视频上传时间**之后，整个预测的结果就和预期的用户点击行为分布基本一致了。（注，在模型预测时，视频的上传时间字段均设置为0）
> 
...
不同于候选生成模型需要对海量的视频进行处理计算，排序模型需要处理的只是候选生成模型经过粗筛得到的小规模视频集合。所以，排序模型可以使用更加复杂的特征作为模型的输入，以期达到更为精准的排序目的。
